...
contract Variables1 {
  string public name = "Example 1"; // State variable with public visibility  
}
...
contract Variables5 {
    ...
    function pay() public payable {
        payer = msg.sender; // Person that intitats transaction
        origin = tx.origin; // Could be a contract calling this
        amount = msg.value; // Valut in this call
    }

    function getBlockInfo() public view returns(uint, uint, uint) {
        return(
            block.number,
            block.timestamp,  // Time in Epoc Time #of seconds since 1/1/1970 Unix Time
            block.chainid     // Which blockchain you are connected to
        );
    }
}
